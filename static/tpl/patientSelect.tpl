<div class="row">
	<div class="col-xs-6">
		<label class="col-sm-4 control-label">Nom</label>
		<input class="col-sm-7" type="text" id="lastname">
	</div>
	<div class="col-xs-6">
		<label class="col-sm-4 control-label">Prénom</label>
		<input class="col-sm-7" type="text" id="firstname">
	</div>
</div>
<div class="row">
	<div class="col-xs-6">
		<label class="col-sm-4 control-label">Code Postal</label>
		<input class="col-sm-7" type="text" id="zipcode">
	</div>
	<div class="col-xs-6">
		<label class="col-sm-4 control-label">Ville</label>
		<input class="col-sm-7" type="text" id="city">
	</div>
</div>
<table class="table table-striped" style="margin-top:30px">
	<thead>
	<tr>
		<th style="min-width:200px">Client</th>
		<th style="width:200px">Périmètre</th>
		<th style="width:200px">Date de naissance</th>
	</tr>
	</thead>
	<tbody>
	<tr>
		<td>Marie Zorn</td>
		<td></td>
		<td></td>
	</tr>
	<tr>
		<td>Albert ducros</td>
		<td></td>
		<td></td>
	</tr>
	<tr>
		<td>Cayce Pollard</td>
		<td></td>
		<td></td>
	</tr>
	<tr>
		<td>Marie Zorn</td>
		<td></td>
		<td></td>
	</tr>
	<tr>
		<td>Albert ducros</td>
		<td></td>
		<td></td>
	</tr>
	<tr>
		<td>Cayce Pollard</td>
		<td></td>
		<td></td>
	</tr>
	<tr>
		<td>Marie Zorn</td>
		<td></td>
		<td></td>
	</tr>
	<tr>
		<td>Albert ducros</td>
		<td></td>
		<td></td>
	</tr>
	<tr>
		<td>Cayce Pollard</td>
		<td></td>
		<td></td>
	</tr>
	<tr>
		<td>Marie Zorn</td>
		<td></td>
		<td></td>
	</tr>
	<tr>
		<td>Albert ducros</td>
		<td></td>
		<td></td>
	</tr>
	<tr>
		<td>Cayce Pollard</td>
		<td></td>
		<td></td>
	</tr>
	</tbody>
</table>