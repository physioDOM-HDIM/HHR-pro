...
var IBeneficiary = {
	...
	getBeneficiariesFilter: function (req, res, next) {
		logger.trace("getBeneficiariesFilter");
		var filter;
		
		jsonParse(req.body)
			.then(function (_filter) {
				filter = _filter;
				return physioDOM.Beneficiaries();
			})
			.then(function (beneficiaries) {
				return beneficiaries.beneficiariesFilter(req.session, filter);
			})
			.then(function (cursor) {
				...
				return new Promise( function(resolve, reject) {
					cursor.each(function (err, item) {
						if (item !== null) {
							result.push({_id: item._id, name: item.name});
						} else {
							resolve(result);
						}
					});
	...
	/**
	 * Post a message to a list of beneficiaries defined by a filter object
	 * 
	 * @param req
	 * @param res
	 * @param next
